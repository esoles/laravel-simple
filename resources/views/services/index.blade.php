...
@section('content')
    <h1 class="text-2xl font-bold mb-4">Servicios de la categoría: <strong class="text-[#0074FF]">{{ $category->name }}</strong></h1>
    ...
    @else
        <div class="max-w-6xl bg-white shadow-md rounded-lg overflow-hidden">
            <div class="grid grid-cols-6 gap-4 bg-[#0074FF] p-4 font-semibold text-white">
                <div>Nombre</div>
                <div>Descripción</div>
                <div>Precio</div>
                <div>Duración</div>
            ...
            @foreach ($services as $service)
                <div class="grid grid-cols-6 gap-4 p-4 border-b">
                    ...
                    <div class="text-gray-600">S/{{ $service->price }}</div>
                    <div class="text-gray-600">{{ $service->duration }}</div>
                    <div class="text-gray-600">{{ $service->position }}</div>
